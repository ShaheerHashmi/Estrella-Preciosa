...
export const GiftModal: React.FC<GiftModalProps> = ({
  ...
  return (
    ...
    >
                ...
                <div className="w-full h-full flex flex-col justify-center">
                  <div className="grid grid-cols-2 gap-2.5 w-full max-h-[60vh] md:max-h-[68vh] overflow-y-auto p-1">
                    {imagesToDisplay.map((src, idx) => (
                      <div
                        key={`multi-img-${idx}`}
                        className="relative group rounded-xl overflow-hidden bg-zinc-950/70 border border-zinc-800 hover:border-amber-500/60 transition-all duration-200 shadow-md h-48 sm:h-56"
                      >
                        <div className="w-full h-full flex items-center justify-center p-2 relative">
                          <img
                            src={src}
                          ...
                          <button
                            onClick={() => setExpandedImage(src)}
                            className="absolute bottom-2 right-2 p-1.5 rounded-lg bg-black/80 text-zinc-300 hover:text-white border border-zinc-700 opacity-0 group-hover:opacity-100 transition-opacity shadow"
                            title="Enlarge Image"
                          >
                            <Maximize2 className="w-3.5 h-3.5" />
                          </button>
          ...
          <div className="md:col-span-6 lg:col-span-5 flex flex-col justify-between p-5 sm:p-7 md:overflow-y-auto space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  {gift.title || `Gift for Day ${panel.id}`}
                </h2>
              </div>

              {/* 2. Origin Section */}
              <div
                id="gift-origin-section"
                className="rounded-xl bg-zinc-900/80 border border-zinc-800/90 p-4 flex items-start space-x-3.5 shadow-sm"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm sm:text-base font-medium text-zinc-200 leading-snug">
                    {gift.origin || 'St. Giles Cathedral, Edinburgh'}
                  </p>
                </div>
              </div>

              {/* 3. Story Behind It Section */}
              <div
                id="gift-story-section"
                className="rounded-xl bg-zinc-900/60 border border-zinc-800/80 p-4 sm:p-5 flex items-start space-x-3.5 shadow-sm"
              >
                <div className="p-2 rounded-lg bg-amber-500/10 text-amber-400 shrink-0 mt-0.5 border border-amber-500/20">
                ...
                <div className="flex-1 min-w-0 space-y-2">
                  ...
                  <p className="text-sm sm:text-base text-zinc-300 leading-relaxed font-sans whitespace-pre-line">
                    {gift.story ||
                      `As Panel ${panel.id} is illuminated in the stained glass window, this gift commemorates a special chapter and milestone in the 21st birthday journey.`}
                  </p>
                </div>
              </div>
            </div>
      ...
      {expandedImage && (
        <div
          className="fixed inset-0 z-60 bg-black/95 flex items-center justify-center p-4 sm:p-8 animate-fade-in"
          onClick={() => setExpandedImage(null)}
        >
          <button
            onClick={() => setExpandedImage(null)}
            className="absolute top-4 right-4 p-2 text-zinc-400 hover:text-white bg-zinc-900/80 hover:bg-zinc-800 rounded-full border border-zinc-700 transition-colors z-70"
